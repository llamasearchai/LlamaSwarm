...
class ResourceGatheringEnv(BaseEnv):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=0, high=max(grid_size, max_energy), shape=(obs_dim,), dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        if action == 0:  # Up
            ...
            new_pos = (
                self.agent_pos[0],
                min(self.grid_size - 1, self.agent_pos[1] + 1),
            )
            self.agent_pos = new_pos
            reward -= 0.1
        elif action == 2:  # Down
            new_pos = (
                min(self.grid_size - 1, self.agent_pos[0] + 1),
                self.agent_pos[1],
            )
            self.agent_pos = new_pos
            reward -= 0.1
        ...
        for resource_type in range(self.num_resource_types):
            for r_idx, (pos, active) in enumerate(self.resources[resource_type]):
                if (
                    not active
                    and self.np_random.random() < self.resource_respawn_probability
                ):
...
def parse_args():
    ...
    parser.add_argument(
        "--num-resource-types", type=int, default=3, help="Number of resource types"
    )
    parser.add_argument(
        "--max-resources", type=int, default=5, help="Maximum number of each resource"
    )
    parser.add_argument(
        "--max-energy", type=int, default=100, help="Maximum energy level"
    )
    parser.add_argument(
        "--energy-decay", type=float, default=1.0, help="Energy decay rate"
    )
    parser.add_argument(
        "--respawn-prob", type=float, default=0.05, help="Resource respawn probability"
    )
    parser.add_argument(
