...
def parse_args():
    ...
    parser.add_argument("--num-obstacles", type=int, default=15, help="Number of obstacles")
    parser.add_argument("--obstacle-penalty", type=float, default=-1.0, help="Penalty for hitting obstacles")
    parser.add_argument("--goal-reward", type=float, default=10.0, help="Reward for reaching the goal")
    parser.add_argument("--step-penalty", type=float, default=-0.1, help="Penalty for each step")
    ...
    parser.add_argument("--epsilon-end", type=float, default=0.1, help="Final epsilon for exploration")
...
def visualize_policy(env, agent, episode, log_dir):
    ...
    plt.savefig(os.path.join(log_dir, "visualizations", f"policy_episode_{episode}.png"))
    plt.close()
...
def main():
    ...
    def episode_callback(episode, stats):
        ...
        # Print progress
        if episode % 50 == 0:
            print(f"Episode {episode}: Reward = {stats['episode_reward']:.2f}, Length = {stats['episode_length']}")
