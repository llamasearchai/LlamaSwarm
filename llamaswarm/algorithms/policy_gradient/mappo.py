...
class MAPPO(BaseAlgorithm):
    ...
    def __init__(
        ...
        super(MAPPO, self).__init__(
            n_agents, state_dim, action_dim, discrete, lr, gamma, device
        )
    ...
    def _init_agents(self):
        ...
        for _ in range(self.n_agents):
            actor = ActorNetwork(
                self.state_dim, self.action_dim, discrete=self.discrete
            ).to(self.device)
            actors.append(actor)
    ...
    def _init_optimizer(self):
        ...
        if self.centralized_critic:
            critic_optimizer = optim.Adam(
                self.agents["critic"].parameters(), lr=self.lr
            )
        else:
            critic_optimizer = []
    ...
    def update(self, batch):
        ...
        for agent_idx in range(self.n_agents):
            ...
            if self.centralized_critic:
                ...
                critic_input = processed_batch["states"].reshape(
                    processed_batch["states"].shape[0], -1
                )
    ...
    def _compute_advantages_and_returns(self, batch):
        ...
        if self.centralized_critic:
            ...
            next_values = torch.zeros(
                (states.shape[0], self.n_agents), device=self.device
            )
        ...
        for t in reversed(range(deltas.shape[0])):
            lastgaelam = (
                deltas[t] + self.gamma * self.gae_lambda * (1 - dones[t]) * lastgaelam
            )
    ...
    def _get_save_dict(self):
        ...
        save_dict = {
            "agents": {
                "actors_state_dict": [
                    actor.state_dict() for actor in self.agents["actors"]
                ]
            },
            "hyperparams": {
        ...
        if self.centralized_critic:
            save_dict["agents"]["critic_state_dict"] = self.agents[
                "critic"
            ].state_dict()
        else:
    ...
    def _set_load_dict(self, checkpoint):
        ...
        if "agents" in checkpoint:
            ...
            if "actors_state_dict" in checkpoint["agents"]:
                for i, state_dict in enumerate(
                    checkpoint["agents"]["actors_state_dict"]
                ):
