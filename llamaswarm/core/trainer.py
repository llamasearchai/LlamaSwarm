...
class Trainer:
    ...
    def train(self, replay_buffer=None):
        ...
        for episode in tqdm(range(self.n_episodes)):
            ...
            if (episode + 1) % self.log_interval == 0:
                avg_reward = np.mean(self.training_metrics['episode_rewards'][-self.log_interval:])
                avg_length = np.mean(self.training_metrics['episode_lengths'][-self.log_interval:])
                print(f"Episode {episode + 1}/{self.n_episodes} - Avg. Reward: {avg_reward:.2f}, Avg. Length: {avg_length:.2f}")
            ...
            if (episode + 1) % self.eval_interval == 0:
                ...
                print(f"Evaluation - Avg. Reward: {np.mean(eval_rewards):.2f}, Avg. Length: {eval_length:.2f}")
    ...
    def _train_episode(self, replay_buffer=None):
        ...
        for agent_losses in losses:
            if agent_losses:
                avg_loss = {k: np.mean([l[k] for l in agent_losses]) for k in agent_losses[0]}
