...
class CooperativeNavigation(MultiAgentEnv):
    ...
    def reset(self):
        ...
        self.agent_positions = np.random.uniform(
            low=0, high=np.array(self.grid_size), size=(self.n_agents, 2)
        )
    ...
    def step(self, actions):
        ...
        for i in range(self.n_agents):
            ...
            self.agent_positions[i] = np.clip(
                self.agent_positions[i], 0, np.array(self.grid_size)
            )
    ...
    def _compute_rewards(self):
        ...
        landmark_distances = np.linalg.norm(
            self.agent_positions - self.landmarks, axis=1
        )
    ...
    def get_obs(self):
        ...
        for i in range(self.n_agents):
            ...
            obs = np.concatenate([own_pos, own_vel, landmark_vec, *other_agents])
    ...
    def get_rewards(self, state, actions):
        ...
        # Calculate distances to landmarks
        landmark_distances = np.linalg.norm(agent_positions - landmarks, axis=1)
    ...
    def render(self, mode="human"):
        ...
        for i, pos in enumerate(self.agent_positions):
            ...
            pygame.draw.circle(
                self.screen, color, screen_pos, int(self.agent_radius * scale_x)
            )

            # Draw a line to the target landmark
            landmark_pos = grid_to_screen(self.landmarks[i])
            pygame.draw.line(self.screen, color, screen_pos, landmark_pos, 1)
