...
class MultiAgentEnv(ABC):
    ...
    def get_agent_obs(self, agent_id):
        ...
        if agent_id < len(observations):
            ...
            raise ValueError(f"Agent ID {agent_id} out of range (max {len(observations)-1})")
    ...
    def get_agent_reward(self, agent_id, state, action):
        ...
        """
        rewards = self.get_rewards(state, [action if i == agent_id else None 
    ...
    def get_env_info(self):
        ...
        return {
            ...
            'action_shapes': [space.shape if hasattr(space, 'shape') else (1,) 
