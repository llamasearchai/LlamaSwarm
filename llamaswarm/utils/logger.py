...
class Logger:
    ...
    def log_episode_summary(
        self, episode, episode_reward, episode_length, losses=None, eval_reward=None
    ):
        """
            ...
            for key, step_dict in self.step_metrics.items():
                if step_dict:
                    step_df = pd.DataFrame(
                        {
                            "step": list(step_dict.keys()),
                            "value": list(step_dict.values()),
                        }
                    )
                    step_df.to_csv(
                        os.path.join(self.csv_dir, f"{key}.csv"), index=False
                    )
    ...
    def plot_metric(
        self, key, title=None, xlabel="Steps", ylabel=None, window=1, save=True
    ):
        """
        ...
        if save:
            ...
            fig.savefig(
                os.path.join(self.plot_dir, filename), dpi=100, bbox_inches="tight"
            )
