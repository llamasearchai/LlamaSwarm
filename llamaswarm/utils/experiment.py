    ...
    def create_run(self, run_name=None, config_updates=None, seed=None):
        ...
        if config_updates:
            for key, value in config_updates.items():
                if (
                    isinstance(value, dict)
                    and key in run_config
                    and isinstance(run_config[key], dict)
                ):
        ...
        for i, values in enumerate(param_values):
            ...
            for seed_idx, seed in enumerate(seeds):
                ...
                run_id, run_config, run_dir, logger = self.create_run(
                    run_name=run_name, config_updates=param_dict, seed=int(seed)
                )
                ...
                try:
                    ...
                    run_results.update(
                        {"params": param_dict, "run_id": run_id, "seed": int(seed)}
                    )
        ...
        try:
            ...
            for result in results:
                ...
                for key, value in result.items():
                    if key not in ["params", "run_id", "seed"] and not isinstance(
                        value, dict
                    ):
                        data[key] = value
